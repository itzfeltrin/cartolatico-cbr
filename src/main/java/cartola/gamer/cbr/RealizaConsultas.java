    ...
    public RetornoModelo retornaConsulta(CaseBaseDescription gameState) throws ExecutionException{
        ...
        for(int i = 0; i < listaCasosRetornados.size(); i++){
            ...
            if(hashCasosRecuperados.containsKey(casoRetornado.getTipoIncidente()))
                hashCasosRecuperados.put(casoRetornado.getTipoIncidente(), hashCasosRecuperados.get(casoRetornado.getTipoIncidente() + 1));
            else hashCasosRecuperados.put(casoRetornado.getTipoIncidente(), hashCasosRecuperados.get(1));
        }
